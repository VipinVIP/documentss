...
void passOne(char label[10], char opcode[10], char operand[10], char code[10],
  ...
  if (strcmp(opcode, "START") == 0) {
    ...
    start = atoi(operand); // convert operand value from string to integer and
                           // assign to start
    locctr = start;
    fprintf(fp4, "\t%s\t%s\t%s\n", label, opcode,
            operand); // write to output file (additional tab space as start
                      // will not have any locctr)
  ...
  while (strcmp(opcode, "END") != 0) {
    ...
    while (strcmp(code, "END") != 0) {
      if (strcmp(opcode, code) == 0) { // if opcode in input matches the one in
                                       // optab, increment locctr by 3
        locctr += 3;
        break;
      }
      // read next line
    ...
    // RESW -> add 3*operand to locctr
      ...
      locctr +=
          (3 *
           (atoi(operand))); // convert operand to integer and multiply with 3
    }
